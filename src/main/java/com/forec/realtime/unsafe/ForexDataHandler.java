...
public class ForexDataHandler extends MatrixHandler{
	...
	public ForexDataHandler(int rows, String currency) {
		super(rows, 6);
	...
	public synchronized boolean update(long lastValue, long timestamp){
		...
		if(timestamp < lastTimestamp){
			throw new IllegalArgumentException("an update can't occur in the past: timestamp is minor than "
					+ "last timestamp ");
		}
		...
		if(get(indexof_lastRecord, OPEN) == UNSET) set(indexof_lastRecord,0,lastValue); 
